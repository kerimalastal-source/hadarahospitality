...
const wrap = (text, max=78) => {
  const words = ascii(text).split(' '); const lines=[]; let line='';
...
function makePdf(product) {
  ...
  const lines = [];
  const text = (x,y,size,value,bold=false) => lines.push(`BT /F${bold?2:1} ${size} Tf ${x} ${y} Td (${pdfEsc(value)}) Tj ET`);
  const rule = (y) => lines.push(`0.78 0.62 0.28 RG 48 ${y} m 547 ${y} l S`);
  lines.push('0.12 0.17 0.23 rg 0 742 595 100 re f');
  text(48, 802, 22, 'HADARA', true); text(48, 783, 9, 'HOSPITALITY  |  ISTANBUL, TURKIYE');
  text(48, 714, 9, 'TECHNICAL SHEET', true); text(48, 680, 22, product.name, true); text(48, 657, 11, category);
  rule(638);
  let y=610;
  const field=(label,value)=>{ text(48,y,9,label.toUpperCase(),true); y-=17; for(const l of wrap(value,82)){text(48,y,10,l); y-=14;} y-=10; };
  ...
  text(48,y,9,'KEY FEATURES',true); y-=19;
  for(const f of product.features){ for(const l of wrap(`- ${f}`,80)){text(55,y,10,l); y-=14;} }
  y-=8; rule(y); y-=24;
  text(48,y,9,'PRODUCTION & SUPPLY',true); y-=18;
  for(const l of wrap('Supplied through trusted Turkish manufacturing partners with hospitality-focused production standards. Customized production, private labeling and bulk supply solutions are available upon request.',82)){text(48,y,9,l); y-=13;}
  ...
  let pdf='%PDF-1.4\n'; const offsets=[0];
  for(let i=1;i<=6;i++){offsets[i]=Buffer.byteLength(pdf);pdf+=`${i} 0 obj\n${objects[i]}\nendobj\n`;}
